-- Criar tabela para armazenar textos personalizados do site
...
-- Função para dropar políticas de forma segura
DO $$
BEGIN
    -- Tentar dropar políticas existentes
    BEGIN
        DROP POLICY "Allow read access to authenticated users" ON site_texts;
    EXCEPTION
        WHEN undefined_object THEN
            -- Política não existe, continuar
            NULL;
    END;
    
    BEGIN
        DROP POLICY "Allow write access to admins and superusers" ON site_texts;
    EXCEPTION
        WHEN undefined_object THEN
            -- Política não existe, continuar
            NULL;
...
-- Política para permitir escrita apenas para admins e superusers
...
-- Dropar trigger existente de forma segura
DO $$
BEGIN
...
-- Inserir textos padrão em português
