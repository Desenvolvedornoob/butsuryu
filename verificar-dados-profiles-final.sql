-- Verificar Dados da Tabela Profiles - VERSÃO FINAL
-- Este script verifica se há problemas específicos com os dados

-- 1. Verificar perfis com dados nulos ou vazios (sem causar erro de enum)
SELECT 
    ...
    COUNT(CASE WHEN role IS NULL THEN 1 END) as role_nulo,
    COUNT(CASE WHEN status IS NULL THEN 1 END) as status_nulo
...
-- 4. Verificar perfis com roles vazias
SELECT 
...
-- 5. Verificar perfis com status vazio
SELECT 
...
-- 6. Verificar perfis com roles inválidas (que não estão no enum)
SELECT 
...
AND role != ''
AND role NOT IN ('admin', 'superuser', 'funcionario')
GROUP BY role;

-- 7. Verificar perfis com status inválido (que não estão no enum)
SELECT 
...
-- 8. Verificar perfis duplicados (mesmo ID)
SELECT 
...
-- 9. Verificar perfis com datas inválidas
SELECT 
...
-- 10. Verificar perfis órfãos (sem usuário correspondente em auth.users)
SELECT 
...
-- 11. Verificar usuários sem perfil (na tabela auth.users mas não em profiles)
SELECT 
...
-- 12. Mostrar uma amostra detalhada dos dados
SELECT 
