-- Verificar Dados da Tabela Profiles - CORRIGIDO
...
-- 4. Verificar perfis com roles inválidas (verificar se há roles vazias)
SELECT 
...
-- 5. Verificar perfis com roles inválidas (que não estão no enum)
SELECT 
...
-- 6. Verificar perfis com status inválido
SELECT 
...
-- 7. Verificar perfis duplicados (mesmo ID)
SELECT 
...
-- 8. Verificar perfis com datas inválidas
SELECT 
...
-- 9. Verificar perfis órfãos (sem usuário correspondente em auth.users)
SELECT 
...
-- 10. Verificar usuários sem perfil (na tabela auth.users mas não em profiles)
SELECT 
...
-- 11. Mostrar uma amostra detalhada dos dados
SELECT 
