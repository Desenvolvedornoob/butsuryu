-- Testar a lógica final corrigida
-- Simular exatamente o que o JavaScript deve calcular
...
final_javascript_simulation AS (
  SELECT 
    td.test_date,
    td.description,
    ...
    FLOOR(EXTRACT(EPOCH FROM (td.test_date - DATE_TRUNC('year', td.test_date))) / (7 * 24 * 60 * 60)) + 1 as week_number_js_final,
...
SELECT 
  ...
  description,
  days_since_start,
  week_number_js_final as js_week_final,
  week_number_postgres as postgres_week,
  js_result_final,
  postgres_result,
  CASE 
    WHEN js_result_final = postgres_result THEN '✅ CORRETO'
    ELSE '❌ DIFERENTE'
  END as status,
  CASE 
    WHEN js_result_final = 2 THEN 'Par - Turno 2 (Grupo A PODE solicitar)'
    ELSE 'Ímpar - Turno 1 (Grupo A BLOQUEADO)'
  END as resultado_grupo_a
FROM final_javascript_simulation
ORDER BY test_date;
