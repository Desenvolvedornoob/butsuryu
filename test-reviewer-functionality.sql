-- ===============================================
-- TESTE DA FUNCIONALIDADE DE REGISTRO DE REVISOR
-- ===============================================
...
BEGIN
    ...
    IF test_user_id IS NOT NULL AND admin_user_id IS NOT NULL THEN
        ...
        -- Verificar o resultado (usando PERFORM para descartar o resultado)
        PERFORM 
            to_user.first_name || ' ' || to_user.last_name as solicitante,
            t.reason,
            t.status,
            reviewer.first_name || ' ' || reviewer.last_name as revisor,
            t.reviewed_at
        FROM public.time_off t
        JOIN public.profiles to_user ON t.user_id = to_user.id
        LEFT JOIN public.profiles reviewer ON t.approved_by = reviewer.id OR t.rejected_by = reviewer.id
        WHERE t.id = test_request_id;
...
-- Verificar solicitações com informações de revisor
SELECT 
    t.id,
